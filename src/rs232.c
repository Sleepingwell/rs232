...
void /*S int*/ OpenComport(int comport_number, int baudrate)
{
  int baudr;

  if((comport_number>21)||(comport_number<0))
  {
    error("illegal comport number"); //S
    //S printf("illegal comport number\n");
    //S return(1);
  }
  ...
  {
    error("unable to open comport"); //S
    //S perror("unable to open comport ");
    //S return(1);
  }

  errorCode = tcgetattr(Cport[comport_number], old_port_settings + comport_number);
  if(errorCode==-1)
  {
    close(Cport[comport_number]);
    error("unable to read portsettings"); //S
    //S perror("unable to read portsettings ");
    //S return(1);
  }
  ...
  /*S error*/errorCode = tcsetattr(Cport[comport_number], TCSANOW, &new_port_settings);
  if(/*S error*/errorCode==-1)
  {
    close(Cport[comport_number]);
    error("unable to adjust portsettings"); //S
    //S perror("unable to adjust portsettings ");
    //S return(1);
  }
...
void /*S int*/ OpenComport(int comport_number, int baudrate)
{
  ...
  switch(baudrate)
  {
    ...
    case  256000 : strcpy(baudr, "baud=256000 data=8 parity=N stop=1");
                   break;
    default      : error("invalid baudrate"); //S printf("invalid baudrate\n");
                   //S return(1);
                   break;
  }
  ...
  {
    error("unable to open comport"); //S
    //S printf("unable to open comport\n");
    //S return(1);
  }
  ...
  if(!BuildCommDCBA(baudr, &port_settings))
  {
    ...
    error("unable to set comport dcb settings"); //S
    //S printf("unable to set comport dcb settings\n");
    //S CloseHandle(Cport[comport_number]);
    //S return(1);
  }
  ...
  {
    CloseHandle(Cport[comport_number]);
    error("unable to set comport cfg settings"); //S
    //S printf("unable to set comport cfg settings\n");
    //S CloseHandle(Cport[comport_number]);
    //S return(1);
  }
  ...
  {
    CloseHandle(Cport[comport_number]);
    error("unable to set comport time-out settings\n"); //S
    //S printf("unable to set comport time-out settings\n");
    //S CloseHandle(Cport[comport_number]);
    //S return(1);
  }
